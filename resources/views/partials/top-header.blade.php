        ...
        <div class="d-flex align-items-center">
            ...
            <div class="search-bar p-0 d-none d-md-block ms-2">
                <div id="search" class="menu-search mb-0">
                         <form role="search" method="get" id="searchform" class="searchform" action="{{ route('search') }}" enctype="multipart/form-data">
                    @csrf

                        <div class='d-flex'>
                            <input type="text" class="form-control border rounded" name="search" id="s" placeholder="@lang('adminPanel.search-top-header') ">
        ...
        <ul class="list-unstyled mb-0">
            ...
            <div class="dropdown dropdown-primary">
                <a class="nav-link dropdown-toggle"  data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="flag-icon @if (App::getLocale()=='en') flag-icon-us @else flag-icon-sa @endif"></i>
                    {{ Config::get('languages')[App::getLocale()] }}
                </a>
                <div class="dropdown-menu menu-user dd-menu dropdown-menu-end bg-white shadow border-0 mt-3 py-3" style="min-width: 200px;">
                    @foreach (Config::get('languages') as $lang => $language)
                    @if ($lang != App::getLocale())

                    <a class="dropdown-item preview-item px-3 py-0" href="{{ route('lang.switch', $lang) }}">
                        <div class="preview-thumbnail">
                            <div class="preview-icon">
                            <i class="flag-icon @if (App::getLocale()=='en') flag-icon-sa @else flag-icon-us @endif"></i>
                    </div>
                        </div>
            ...
            <li class="list-inline-item mb-0 ms-1">
                <div class="dropdown dropdown-primary">
                    <button type="button" id="btnMenuUser" class="btn btn-soft-light dropdown-toggle p-0" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <img src="{{auth()->user()->image}}" class="avatar avatar-ex-small rounded" alt="">
                    </button>
                    <div class="dropdown-menu menu-user dd-menu dropdown-menu-end bg-white shadow border-0 mt-3 py-3" style="min-width: 200px;">
                        <a class="dropdown-item d-flex align-items-center text-dark pb-3" href="@if(auth()->user()->user_type == 'admin' || auth()->user()->user_type == 'admin_employee')
           ...
           @endif">
                        ...
                        @elseif (auth()->user()->user_type == 'store_admin' || auth()->user()->user_type == 'store_employee')
                        {{route('store.profile')}}
                        @endif"><span class="mb-0 d-inline-block me-1"><i class="ti ti-settings"></i></span> @lang('adminPanel.profile') </a>
                        <a class="dropdown-item text-dark" href="{{ route('logout') }}"
                                   onclick="event.preventDefault();
                                                     document.getElementById('logout-form').submit();"><span class="mb-0 d-inline-block me-1"><i class="ti ti-logout"></i></span>  @lang('adminPanel.logout') </a>
