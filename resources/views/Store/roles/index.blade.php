    ...
    <div class="layout-specing">
        ...
        <div class="bg-white mt-3">
            <div class="d-flex justify-content-between p-4  bg-white   rounded-top border-bottom heading-with-shape">
                ...
                <div class="btns">
                @if(auth()->user()->can('store-roles-create'))
                    <a href="{{route('store.roles.create')}}"  class="btn btn-dark">@lang('adminPanel.add-role') </a>
                @endif
                </div>
            </div>
...
{{$role->id}}
                                ...
                                <td class="text-end p-3">
                                    ...
                                    @if(auth()->user()->can('store-roles-edit'))
                                        <a href="{{route('store.roles.edit',['id'=>$role->id])}}"> <i class="uil uil-edit"></i> </a>
                                        @endif
                                        @if(auth()->user()->can('store-roles-delete'))
                                        <a href="{{route('store.roles.delete',['id'=>$role->id])}}"  onclick="return confirm('هل انت متأكد من حذف ({{$role->name}})?')" > <i class="uil uil-trash-alt"></i> </a>
                                    @endif
                                    </div>
                                </td>
                            </tr>
