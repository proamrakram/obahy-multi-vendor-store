    ...
    <div class="layout-specing">
        ...
        <div class="bg-white mt-3">
            <div class="d-flex justify-content-between p-4  bg-white   rounded-top border-bottom heading-with-shape">
                <h6 class="fw-bold mb-0 h4">@lang('adminPanel.cities')</h6>
                <div class="btns">
                @if(auth()->user()->can('cities-create'))
                    <a href="{{route('admin.cities.create')}}"  class="btn btn-dark">@lang('adminPanel.add-city') </a>
                @endif
                </div>
            </div>
                    ...
                    <table class="table table-bordered table-center table-hover bg-white mb-0" id="kt_datatable_teachers">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center"> @lang('adminPanel.city-name')</th>
...
{{$city->id}}
                                ...
                                <td class="text-end p-3">
                                    ...
                                    <div class="form-check form-switch p-0 pt-1">
                                            ...
                                            <input class="form-check-input" type="checkbox" onclick="window.location='{{ route("admin.cities.changeStatus",["id"=>$city->id]) }}'" id="flexSwitchCheckChecked" @if($city->status == 'active') checked="" @endif>
                                            
                                        </div>
                                        @if(auth()->user()->can('cities-edit'))
                                        <a href="{{route('admin.cities.edit',['id'=>$city->id])}}"> <i class="uil uil-edit"></i> </a>
                                        @endif
                                        @if(auth()->user()->can('cities-delete'))
                                        <a href="{{route('admin.cities.delete',['id'=>$city->id])}}"  onclick="return confirm('هل انت متأكد من حذف ({{$city->city_name_ar}})?')" > <i class="uil uil-trash-alt"></i> </a>
                                    @endif
                                    </div>
                                </td>
                            </tr>
