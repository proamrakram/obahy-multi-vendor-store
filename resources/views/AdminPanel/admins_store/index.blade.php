    ...
    <div class="layout-specing">
        ...
        <div class="bg-white mt-3">
            <div class="d-flex justify-content-between p-4  bg-white   rounded-top border-bottom heading-with-shape">
                <h6 class="fw-bold mb-0 h4"> @lang('adminPanel.store-manager-settings') </h6>
                <div class="btns">
                    
            @if(auth()->user()->can('admins-store-create'))
                ...
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content rounded shadow border-0">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('admin.admins-store.store') }}" method="POST">
                                @csrf
            ...
            <div class="modal fade" id="editUser" tabindex="-1" aria-labelledby="LoginForm-title" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content rounded shadow border-0">
                        <div class="modal-header border-bottom bg-soft-dark">
                            <h5 class="modal-title" id="LoginForm-title"> @lang('adminPanel.edit-store-manager-data') </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                        <div id="modal-body">
                    ...
                    <table class="table table-bordered table-center table-hover bg-white mb-0" id="kt_datatable_teachers">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center p-2">
                                    @if($admin->user_status =='active')<span class="text-success"> @lang('adminPanel.active') @else <span class="text-danger"> @lang('adminPanel.not-active')  @endif</span>
                                </td>
                                <td class="text-center p-2">
                                ...
                                <td class="text-end p-3">
                                    <div class="tools-options d-flex justify-content-center">
                                        <div class="form-check form-switch p-0 pt-1">
                                            ...
                                            <input class="form-check-input" type="checkbox" onclick="window.location='{{ route("admin.admins-store.changeStatus",["id"=>$admin->id]) }}'" id="flexSwitchCheckChecked" @if($admin->user_status == 'active') checked="" @endif>
                                            
                                        </div>
                                        @if(auth()->user()->can('admins-store-edit'))
                                        <a href="#" onclick="return func({{$admin->id}})"> <i class="uil uil-edit"></i> </a>
                                        @endif
                                        @if(auth()->user()->can('admins-store-delete'))
                                        <a href="{{route('admin.admins-store.delete',['id'=>$admin->id])}}" onclick="return confirm('هل انت متأكد من حذف ({{$admin->name}})?')"> <i class="uil uil-trash-alt"></i> </a>
                                    @endif
                                    </div>
                                </td>
                            </tr>
...
<script>
    ...
    function func_change_status(id) {
        $.ajax({
            url: '/admin/admins-store/change-status',
            dataType: 'html',
            method: 'GET',
            data: {
                id: id
...
var KTDatatablesExample = function () {
    ...
    // Hook export buttons
    var exportButtons = () => {
        const documentTitle = 'مديري المتاجر';
