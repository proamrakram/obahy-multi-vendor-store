    ...
    <div class="layout-specing">
        ...
        <div class="bg-white mt-3">
            <div class="d-flex justify-content-between p-4  bg-white   rounded-top border-bottom heading-with-shape">
                ...
                <div class="btns">
                @if(auth()->user()->can('admin-products-comments-create'))
                    <a href="{{route('admin.products-comments.create')}}"  class="btn btn-dark"> @lang('adminPanel.add-comment')  </a>
               @endif
                </div>
            </div>
                            ...
                            @endfor
                                ...
                                <td class="text-end p-3">
                                    ...
                                    <div class="form-check form-switch p-0 pt-1">
                                            ...
                                            <input class="form-check-input" type="checkbox" onclick="window.location='{{ route("admin.products-comments.changeStatus",["id"=>$comment->id]) }}'" id="flexSwitchCheckChecked" @if($comment->status == 'active') checked="" @endif>
                                            
                                        </div>
                                        @if(auth()->user()->can('admin-products-comments-edit'))
                                        <a href="{{route('admin.products-comments.edit',['id'=>$comment->id])}}"> <i class="uil uil-edit"></i> </a>
                                        @endif
                                        @if(auth()->user()->can('admin-products-comments-delete'))
                                        <a href="{{route('admin.products-comments.delete',['id'=>$comment->id])}}"  onclick="return confirm('هل انت متأكد من حذف التعليق?')" > <i class="uil uil-trash-alt"></i> </a>
                                    @endif
                                    </div>
                                </td>
                            </tr>
