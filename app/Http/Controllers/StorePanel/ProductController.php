...
namespace App\Http\Controllers\StorePanel;
...
class ProductController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('StorePanel.Product.index')->with('category', $category);
    }
    ...
    public function add_service_made()
    {
        ...
        return view('StorePanel.Product.product_service_made')->with('category', $category)
    ...
    public function add_custom_made()
    {
        ...
        $product_color = ProductColor::where('is_delete', 0)->orderBy('created_at', 'desc')->get();
        // echo json_encode($product_color);die;
        return view('StorePanel.Product.product_custom_made')->with('category', $category)
            ->with('size_option', $size_option)
            ->with('product_color', $product_color);
    }
    ...
    public function add_ready_made()
    {
        ...
        $product_color = ProductColor::where('is_delete', 0)->orderBy('created_at', 'desc')->get();
        // echo json_encode($product_color);die;
        return view('StorePanel.Product.product_ready_made')->with('category', $category)
            ->with('size_option', $size_option)
            ->with('product_color', $product_color);
    }
            ...
            if ($product->product_type == 'custom_made') {
             ...
             foreach($color as $c){
               $arr[] = $c->color_id;
                ...
                $category = ProductCategory::where('is_delete',0)->where('parent_id',0)->get();
                $sub_category = ProductCategory::where('is_delete',0)->where('parent_id','!=',0)->get();

                $custom = ProductCustomMade::where('product_id', $product->id)->first();
                return view('StorePanel.Product.edit_custom_product')
    ...
    public function update_product(Request $request, $id)
    {
        ...
        $this->validate(
            ...
            [
                ...
                // 'product_description_en' => 'required|string|min:50',
                // 'product_description_ar' => 'required|string|min:50',
        ...
        if ($data['product_type'] == 'custom_made') {
            ...
            $this->validate($request, [
                'custom_made_description' => 'nullable|string|min:50',
                'custom_made_description_en' => 'nullable|string|min:50',
                'fabric_options' => 'nullable|string|min:50',
                'implementation_period' => 'required|string',
                // 'implementation_period_en' => 'required|string|min:50',
                // 'product_description_ar' => 'required|string|min:50',
                'product_type' => 'required|in:ready_made,custom_made,service,template,model',
                'product_category' => 'required|exists:product_categories,id',
    ...
    public function store_product(Request $request)
    {
        ...
        $this->validate(
            ...
            [
                ...
                'product_description_en' => 'nullable|string|min:50',
                'product_description_ar' => 'nullable|string|min:50',
                'is_affiliate' => 'nullable|in:0,1',
                'product_type' => 'required|in:ready_made,custom_made,service,template,model',
                'product_category' => 'required|exists:product_categories,id',
        ...
        if ($data['product_type'] == 'custom_made') {
            ...
            $this->validate($request, [
                'custom_made_description' => 'nullable|string|min:50',
                'fabric_options' => 'nullable|string|min:50',
                'implementation_period' => 'required|string|min:50',
                'custom_made_description' => 'required|string|min:50',
                'product_type' => 'required|in:ready_made,custom_made,service,template,model',
                'product_category' => 'required|exists:product_categories,id',
                'product_colors' => 'required|array',
                'product_3d_image' => 'required',
