...
class ProductsCommentsController extends Controller
{
  ...
  {
    $comments = ProductComment::where('is_delete',0)->get();
    return view('AdminPanel.products_comments.index')->with('comments', $comments);
  }
  ...
  public function create()
  {
    ...
    $products = Product::where('product_status', 'active')->where('is_delete',0)->get();
    return view('AdminPanel.products_comments.create')->with('users',$users)->with('products',$products);
  }
  ...
  public function edit($id)
  {
    ...
    if(is_null($comment) || $comment->is_delete == 1) {
    ...
    $users = User::where('user_status','active')->where('user_type', 'customer')->where('is_delete',0)->get();
    $products = Product::where('product_status', 'active')->where('is_delete',0)->get();
    return view('AdminPanel.products_comments.edit')->with('comment', $comment)->with('users',$users)->with('products',$products);
  }
